 ...
 *
 * 使用方法：
 *   1. 在 AutoX.js 中导入并运行此脚本。
 *   2. 确保已授予**无障碍服务权限**和**悬浮窗权限**。
 *   3. 修改下方 `CONFIG` 对象中的配置项以适配你的需求。
 *   4. 点击运行，浮窗会显示带时间戳的实时日志。
 *
 * 需配置的变量（CONFIG 内）：
...
function dumpPageInfo(label) {
    ...
    try {
        ...
        var count = Math.min(all.length, 40);
    ...
    tlog("=== " + label + " 结束 (" + Math.min(all ? all.length : 0, 40) + "条) ===");
}
